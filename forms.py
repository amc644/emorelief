...
class wtfProfile(FlaskForm):
    fName=StringField('First name',validators=[InputRequired('The first name field is required'),Length(min=2,max=30,message='The first name must be 2 to 30 characters long.')])
    mName=StringField('Middle Name')
    lName=StringField('Last name',validators=[InputRequired('The last name field field is required'),Length(min=2,max=30,message='The last name must be 2 to 30 characters long.')])
    cPhone=StringField()
    hPhone=StringField()
    wPhone=StringField()
    email=EmailField('Email',validators=[InputRequired('The email field is required'),Email(),Length(min=2,max=30,message='Incorrect email.')])
    ...
    ssn=StringField()
    dob=DateField()
    usrnm=StringField('Username',validators=[InputRequired('Username field is required'),Length(min=1,max=30,message='Username must be 1 to 30 characters long..')])
    pw= PasswordField('Password',validators=[InputRequired('The passoword field is required'),Length(min=8,max=30,message='The password must contain at least one uppercase letter, one lowercase letter, one number, and one special character.')])